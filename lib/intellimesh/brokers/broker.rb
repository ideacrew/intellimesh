module Intellimesh
  module Brokers
    class Broker
      ...
      attr_reader :adapter, :configuration, :name
      ...
      def initialize(name = DEFAULT_BROKER_NAME, options)
        ...
        @destinations = {}
        @consumers = {}
        logger.debug 'Intellimesh module Brokers configured successfully!'
      end

      # @return [Intellimesh::Brokers::Client] the client module for this broker
      def client
      ...
      def consumer(key, &block)
        raise Intellimesh::Brokers::Error, 'you must provide a block' unless block_given?
        @consumers[key] = block
      end
      ...
      # @raise [Intellimesh::Brokers::NoSuchDestinationError] if there is no destination with that name
      def find_destination(destination_name)
        destination = @destinations[destination_name]
        if destination.nil?
          raise Intellimesh::Brokers::NoSuchDestinationError, "no destination #{destination_name} has been configured"
        end
        destination
      end
      ...
      def find_consumer(consumer_name)
        ...
        raise Intellimesh::NoSuchConsumerError, "no consumer #{consumer_name} has been configured" if consumer.nil?
        consumer
      end
      ...
      def resolve_adapter(adapter, options)
        ...
        when Intellimesh::Brokers::Adapters::Base
          adapter
        else
          raise "adapter must be a Intellimesh module Brokers::Adapters::Base, but this object is a #{adapter.class}"
        end
      end
      ...
      def find_adapter_class(adapter_name)
        ...
        unless respond_to?(adapter_method)
          raise ['the adapter',
                 adapter_name,
                 'must provide',
                 "Intellimesh module Brokers::Broker##{adapter_method}",
