...
module Intellimesh
  class Broker
    def bunny_adapter
      Intellimesh::Adapters::BunnyAdapter
    end
  end
  ...
  module Adapters
    class BunnyAdapter < Base
      ...
      class Destination < Intellimesh::Destination::Base
      ...
      class QueueDestination < Destination
        def after_initialize(adapter_context)
          if @dest_options[:no_declare]
            if @name.empty?
              raise Intellimesh::Error, 'server-named queues must be declared, but you provided :no_declare => true'
            end
            if @dest_options[:bindings]
              raise Intellimesh::Error, 'queues with bindings must be declared, but you provided :no_declare => true'
            end
          else
        ...
        def handle_queue_init(queue)
          ...
          if (bindings = @dest_options[:bindings])
            bindings.each do |bnd|
              raise Intellimesh::Error, "binding #{bnd.inspect} must provide a source!" unless bnd[:source]
        ...
        def after_initialize(adapter_context)
          if (declare = @dest_options[:declare])
            adapter_context.with_channel(false) do |ch|
              ...
              raise Intellimesh::Error, 'you must provide a valid exchange type' unless type
        ...
        def start
          ...
          @message_handler =  case options.delete(:ack)
                              ...
                              when :transactional
                                ...
                                raise Intellimesh::Error, "unrecognized :ack option #{options[:ack]}"
                              end
          start_subscription
        end
      ...
      def connection(ensure_started = true)
        if ensure_started
          begin
            ...
            raise Intellimesh::ConnectionError.new(e.to_s, e)
          rescue => e
            stop
            raise e
          end
      ...
      class BunnyContext < ContextBase
        ...
        def handle_create_destination(name, dest_options = {}, message_props = {})
          dest =  case type = dest_options.delete(:type)
                  ...
                  when :queue, nil
                    ...
                    raise Intellimesh::Error, "invalid destination type #{type}"
                  end
        ...
        def handle_begin_transaction(options = {})
          if in_transaction?
            raise Intellimesh::TransactionError,
        ...
        def handle_pop_message(destination, options = {})
          raise Intellimesh::Error, "You can't pop a message off an exchange" if destination.is_a? ExchangeDestination
        ...
        def handle_errors
          ...
          if e.is_a? Bunny::NotFound
            raise Intellimesh::QueueNotFound.new(e.to_s, e)
          else
            raise Intellimesh::WrappedError.new(e.to_s, e)
          end
        ...
        def with_channel(require_commit = true)
          raise Intellimesh::TransactionRollbackOnly if @rollback_only
          raise Intellimesh::Error, 'this adapter context is not valid!' unless valid?
          ensure_channel
      ...
      def validate_bunny_version
        ...
        unless required.satisfied_by? current
          raise Intellimesh::Error, 'bunny 1.7.0 or later is required for the bunny adapter'
        end
      end
    end
  end
